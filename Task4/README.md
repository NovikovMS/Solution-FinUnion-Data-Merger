...
К концу месяца 3 вводится ограниченный production MVP: минимальный MDM Customer Hub с глобальным `customer_id` и source links, critical RDM/Product mapping, управляемые batch/инкрементальные потоки, PostgreSQL DWH P01, проверки качества и единые витрины клиентов, счетов, продуктов и операций. В месяцах 3–8 эти уже работающие компоненты расширяются до целевых Kafka/CDC, Lakehouse и полных data products, а не создаются впервые.
